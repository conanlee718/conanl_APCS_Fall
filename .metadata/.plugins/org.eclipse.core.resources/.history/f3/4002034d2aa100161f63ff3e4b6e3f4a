...
//APCS 2nd period 
//Arrays Lab 3 
//GOAL :Include methods: sum, append, remove, add even sums, and rotate right.
//GOAL :Manipulating arrays so we can comfortably work with arrays in other coding projects. 
...
public class ArraysLab3 
{
	...
	public static int[] sum(int[] arr1, int[] arr2)
	{
		...
		int[] sumarray=new int[sumarray.length];
	...
	{
		assert (arr.length > 0);
		int[]arrplus=new int[arrplus.length];
		arrplus=arr;
		for(int i=0;i<arrplus.length;i++){
			if(i==arrplus.length-1){
				arr[i+1]=num;
			}
		}
		return(arrplus);
	...
	{
		assert (arr.length >= 2);
		int[]arrremoved=new int[arrremoved.length];
		arrremoved=arr;
	...
	{
		assert (arr.length > 0);
		int[]arrshift=new int[arrshift.length];
		arrshift=arr;
		int first=arrshift[0];
